...
- **CI wiring (2026-08-15, reverted 2026-08-15)** — `.github/workflows/ci.yml` landed briefly (the routing map's last fog item): oxlint, oxfmt `--check`, tsc, vitest unit + browser (playwright chromium install + cache), and the build — on push + PR. **Removed** — it failed; team decided to drop GitHub Actions rather than fix.
