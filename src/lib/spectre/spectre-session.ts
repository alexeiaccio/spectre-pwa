...
export class SpectreSession {
  ...
  private constructor(identity: Identity, masterKey: CryptoKey, version: AlgorithmVersion) {
  ...
  static async unlock(identity: Identity, passphrase: string): Promise<SpectreSession> {
    const raw = await scrypt(encoder.encode(passphrase), userSalt(identity.fullName, identity.algorithm), 64)
    const masterKey = await crypto.subtle.importKey('raw', toBuf(raw), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign'])
    raw.fill(0)
  ...
  async password(site: Site): Promise<string> {
    ...
    const salt = siteKeySalt(this.version, site.name, site.counter, purpose, context)
    const siteKey = new Uint8Array(await crypto.subtle.sign('HMAC', mk, toBuf(salt)))
...
function purposeOf(purpose: Site['purpose'], answer?: string): { purpose: Purpose; context: string | null } {
  switch (purpose) {
    case 'password': return { purpose: 'authentication', context: null }
    case 'login': return { purpose: 'identification', context: null }
    case 'answer': return { purpose: 'recovery', context: answer ?? null }
  }
}