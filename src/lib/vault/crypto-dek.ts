...
/** Encode a string to an ArrayBuffer-backed Uint8Array (satisfies TS 6.0 BufferSource). */
...
/** Copy a Uint8Array into a fresh ArrayBuffer so TS 6.0's BufferSource accepts it. */
...
export const generateDek = (): Effect.Effect<DekAndRaw, CryptoError> =>
  Effect.tryPromise(async () => {
    ...
    return { key, raw }
  }).pipe(Effect.mapError(() => new CryptoError('generateKey failed')))
 ...
 */
const kekFromSecret = (secret: Uint8Array, salt: Uint8Array): Effect.Effect<AesKey, CryptoError> =>
  Effect.tryPromise(async () => {
    const base = await crypto.subtle.importKey('raw', toBuf(secret), 'HKDF', false, ['deriveKey'])
    return crypto.subtle.deriveKey(
      { name: 'HKDF', hash: 'SHA-256', salt: toBuf(salt), info: toBuf(ab(DOMAIN + '/kek')) },
      base,
      { name: 'AES-GCM', length: 256 },
      false,
      ['encrypt', 'decrypt'],
    )
  }).pipe(Effect.mapError(() => new CryptoError('HKDF deriveKey failed')))

/** HKDF of a PRF output (or recovery code bytes) — the KEK used to wrap the DEK. */
export const kekFromPrf = (prfOutput: Uint8Array, salt: Uint8Array): Effect.Effect<AesKey, CryptoError> =>
  kekFromSecret(prfOutput, salt)
...
export const wrapDek = (
  ...
  Effect.tryPromise(async () => {
    ...
    const wrapped = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: toBuf(iv) }, kek, toBuf(raw))
    return { iv, wrapped: new Uint8Array(wrapped) }
  }).pipe(Effect.mapError(() => new CryptoError('wrapKey failed')))
...
export const unwrapDek = (
  ...
  Effect.tryPromise(async () => {
    const raw = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: toBuf(iv) }, kek, toBuf(wrapped))
    return crypto.subtle.importKey('raw', raw, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt'])
  }).pipe(Effect.mapError(() => new CryptoError('unwrapKey failed')))
...
export const encryptBlob = (
  ...
  Effect.tryPromise(async () => {
    const iv = crypto.getRandomValues(new Uint8Array(12))
    const ct = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: toBuf(iv) }, dek, toBuf(plaintext))
    return { iv, ct: new Uint8Array(ct) }
  }).pipe(Effect.mapError(() => new CryptoError('encrypt failed')))

export const decryptBlob = (dek: AesKey, iv: Uint8Array, ct: Uint8Array): Effect.Effect<Uint8Array, CryptoError> =>
  Effect.tryPromise(() =>
    crypto.subtle.decrypt({ name: 'AES-GCM', iv: toBuf(iv) }, dek, toBuf(ct)),
  ).pipe(
    Effect.map((pt) => new Uint8Array(pt)),
    Effect.mapError(() => new CryptoError('decrypt failed — wrong key or corrupt blob')),
  )
